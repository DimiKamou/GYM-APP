...
-- Every "must FAIL" step goes through tests.must_fail() (00_supabase_shim.sql):
-- it prints «σωστό» when the schema refused and «ΛΑΘΟΣ» when it did not, and
-- run.sh fails on the latter. A bare statement that prints ERROR is not an
-- assertion — psql still exits 0 — which is how check 2 passed for months
-- while the row it was meant to refuse was landing every time.
...
\echo '--- 2. Maria tries to forge logged_by as Dimitris on INSERT (must be overwritten to Maria) ---'
-- Not a refusal: sessions_stamp_author() replaces whatever the client sent,
-- and sessions_insert_self then sees Maria. The row lands — as Maria's.
...
select case when logged_by = 'cccccccc-0000-0000-0000-000000000001'
            then 'forged logged_by overwritten to Maria: σωστό'
            else 'ΛΑΘΟΣ: logged_by landed as '||coalesce(logged_by::text,'null') end
  from public.sessions where id='eeeeeeee-0000-0000-0000-000000000009'
union all
select 'ΛΑΘΟΣ: the forged session was not written at all'
 where not exists (select 1 from public.sessions where id='eeeeeeee-0000-0000-0000-000000000009');
